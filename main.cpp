...
static ExprAST *ParseNumberExpr() {
    ExprAST *Result = new NumberExprAST(stod(objarray[x].value));
...
static ExprAST *ParsePrimary() {
    switch (CurTok) {
        ...
        case '[': return ParseIdentifierExpr();
...
vector<double > addition(vector<double> left,vector<double> right){
    vector<double> final;
    for(int i=0;i<left.size();i++){
        final.push_back(left[i]+right[i]);
    }
    return final;
}
///////////////////////////////////////////////////////////////////////////////////////////////
...
vector<double> scalare(vector<double> vec1,double coeficient)
{
    vector<double > vec3;
    for (unsigned i = 0;i<vec1.size();i++)
    {
        vec3.push_back(vec1[i]*coeficient);
    }
    return vec3;
}

vector<double> unit(vector<double> vec1){
    vector<double> vec3=scalare(vec1,1/len(vec1));
    return vec3;
}

vector<double> dot(vector<double> vec1,vector<double> vec2){
    vector<double> toReturn;
    double dot=0;
    for (unsigned i=0;i<vec1.size();i++){
        dot+=vec1[i]*vec2[i];
    }
    toReturn.push_back(dot);
    return toReturn;
}
...
vector<double> getVector(ExprAST *node) {
    BinaryExprAST *binaryExprAst = dynamic_cast<BinaryExprAST *>(node);
    CallExprAST *callExprAst = dynamic_cast<CallExprAST *>(node);
    if(callExprAst){
        vector<double > toReturn;
        for(int i=0;i<callExprAst->Args.size();i++){
            toReturn.push_back(dynamic_cast<NumberExprAST*>(callExprAst->Args[i])->Val);
        }
        return toReturn;
    }
    if(binaryExprAst) {
        vector<double> left = getVector(binaryExprAst->LHS);
        vector<double> right = getVector(binaryExprAst->RHS);
        if(binaryExprAst->Op=='+'){
     ...
     /*   if (left) {
            for (int i = 0; i < left->Args.size(); i++)
                binaryExprAst->vector.push_back(dynamic_cast<NumberExprAST *>(left->Args[i])->Val);
            return binaryExprAst->vector;
        }*/
      //  return getVector(binaryExprAst->LHS);

}
    vector<double> fromBody(ExprAST *exp){
        ExprAST* Body = dynamic_cast<FunctionAST*>(exp)->Body;
        return getVector(Body);
    }
